...
func _boot_domain() -> void:
	...
	zm    = ZoneManager.new()
	add_child(zm)          ## Node needs to be in tree for signal connections
	zm.setup([p1, p2])
	stack = EffectStack.new()
	add_child(stack)
	stack.setup([p1, p2], zm)

	## Register services so EffectResolutionStep inner classes can find them.
...
func _boot_ui() -> void:
	board = BoardView.new()
	add_child(board)
	board.setup(zm, stack, [p1, p2])

	# Connect board signals to test handlers
...
func _spell_def(id: StringName, _name: String, stype: CardDefinition.SpellType) -> CardDefinition:
	var d          := CardDefinition.new()
	d.card_id      = id
	d.card_name    = _name
...
func _trap_def(id: StringName, _name: String, ttype: CardDefinition.TrapType) -> CardDefinition:
	var d          := CardDefinition.new()
	d.card_id      = id
	d.card_name    = _name
...
func _summon_from_hand_enemy() -> void:
	var hand := zm.hand_of(p2)
	if hand.is_empty():
		print("TestBoard: P2 hand is empty")
		return

	## Find first monster in hand
	...
	if target == null:
		print("TestBoard: No normal-summonable monster in P2 hand")
		return

	if zm.monster_zone_of(p1).is_full():
		print("TestBoard: P2 monster zone is full")
		return

	var slot := zm.move_to_first_slot(
		target, zm.monster_zone_of(p2), ZoneManager.MoveReason.NORMAL_SUMMON
	)
	target.record_normal_summon(_turn)
	board.refresh_hand(p2)
	print("TestBoard: Summoned %s to slot %d" % [target.definition.card_name, slot])
func _summon_from_hand() -> void:
	...
	if zm.monster_zone_of(p1).is_full():
		print("TestBoard: P1 monster zone is full")
		return

	var slot := zm.move_to_first_slot(
		target, zm.monster_zone_of(p1), ZoneManager.MoveReason.NORMAL_SUMMON
	)
	target.record_normal_summon(_turn)
...
func _set_spell_from_hand() -> void:
	...
	if zm.spell_zone_of(p1).is_full():
		print("TestBoard: P1 spell/trap zone is full")
		return

	zm.move_to_first_slot(
		target, zm.spell_zone_of(p1), ZoneManager.MoveReason.SET
	)
	board.refresh_hand(p1)
...
func _on_phase_advance() -> void:
	_phase = (_phase + 1) % PHASES.size()
	if _phase == 0:
		_turn += 1
	_update_hud()
	print("TestBoard: Phase → %s (Turn %d)" % [PHASES[_phase], _turn])
...
func _update_hud() -> void:
	...
	board.update_phase(PHASES[_phase], _turn, 1)
